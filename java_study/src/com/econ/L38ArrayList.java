...
public class L38ArrayList {
	...
	public static void main(String[] args) {
		...
		System.out.println();
		byte[] c= {127,1,4}; 
		// 배열은 고정길이 만큼 미리 선언해야한다.(길이가 변하지 않는다.)
		
		System.out.println("Array의 필드는 length 밖에 없다. c.length : "+c.length);
		/* Array는 가볍고 리터럴표기법을 지원해서 선언하기 편하지만
		   지원해주는 함수가 전혀 없다.    */
		...
		System.out.println("int_list의 총합 : "+total);
		...
		System.out.println(int_list); // toString 이 구현되어 있다.
		System.out.println(int_list.remove(3)); 
		// 삭제한 값이 나오고 삭제한 위치만큼 한칸씩 순서를 당긴다(remove)
		...
		list2.add(4);
		list2.add(2, 1000);
		//2번째 자리 이후의 요소를 다 한칸씩 밀고 2번째 자리에 1000을 추가
		list2.set(4, -10); // .set : 특정 인덱스에 요소(값)을 바꾼다.
		...
		//ArrayList<Integer> num_lists=new ArrayList<Integer>(num_list);
