...
class LoginFrame extends Frame{
	...
	public LoginFrame(String name) {
		...
		btnP.add(submit);
		this.add(btnP);
		/* 추상클래스와 인터페이스는 구현이 덜되어서 생성자를 호출 할 수 없지만
		   생성하면서 바디를 구현하면 만들 수 있다. 이때 익명클래스가 생성된다.*/
		//this.addWindowListener(new WindowListener(){});
		/* 인터페이스는 구현해야하는 것이 너무 많아서 인터페이스를 미리 구현한
		   추상클래스인 Adapter가 존재한다. */
		this.addWindowListener(new WindowAdapter() {
			// this 는 WindowAdapter기 때문에 frame을 참조하기위해 필드에 등록함.
